...
limit_index_fn_name: int = 0
functions_definition: List[Dict[str, Any]] = []
limited_tokens: List[Any] = []
model: Any = None


def isValid(s: str) -> bool:
    """Check if the string has balanced curly braces."""
    if not s:
        return False
    new_s = str()
    for ch in s:
...
def set_limited_tokens_for_args(
    functions_definition: List[Dict[str, Any]], target_name: str
) -> List[Any]:
    """Adjust token limits based on target types."""
    new_limited_tokens: List[Any] = []
    list_params_names = {
        name_param: t['type']
        for f in functions_definition
        for name_param, t in f['parameters'].items()
...
def get_masked_logits(
    logits: List[float], next_expected_tokens: Union[int, List[int], None]
) -> Optional[List[float]]:
    """Mask unallowed token logits with negative infinity."""
    if isinstance(next_expected_tokens, int):
        masked_logits = [
            score if index == next_expected_tokens else float('-inf')
            for index, score in enumerate(logits)
        ]
    elif isinstance(next_expected_tokens, list):
        masked_logits = [
            score if index in next_expected_tokens else float('-inf')
            for index, score in enumerate(logits)
        ]
...
def generate_token(
    ...
    """Generate tokens iteratively under constraints."""
    i = 0
    function_name = str()
    flag = True
    answer = ""
    ...
    while not isValid(answer):
        ...
        token_limit = (
            limited_tokens[i]
            if not auto_generation and i < len(limited_tokens)
            else None
        )
        n_logits = get_masked_logits(logits, token_limit)
        if n_logits is None:
            return None
...
def encode_function_name(f_name: str) -> Tuple[int, ...]:
    """Encode and cache the token tuple for a function name."""
    tensor_list_obj = model.encode(f_name)
    return tuple(tensor_list_obj[0].tolist())


def get_functions_data(
    functions: List[Dict[str, Any]], model: Any
) -> List[List[int]]:
    """Get positional token options for function names."""
    ...
    # reverse matrix (rows -> columns and columns -> rows)
    reversed_two_d_tokens = [
        list(set(item for item in t if item))
...
def get_limits_probabs(
    new_tokens: List[Any], matrix_function_names: List[List[int]]
) -> List[Any]:
    """Inject positional constraints into token pipeline."""
...
def constrained_decoding(
    ...
    """Build structural token restrictions."""
    format_json = (
        '{"name":"<|im_start|>fn<|im_end|>",'
        '"args":{<|im_start|>args<|im_end|>}}'
    )
...
def get_vocabs(model: Small_LLM_Model) -> Dict[str, int]:
    """Load vocabulary mappings from model files."""
    vocab_path = model.get_path_to_tokenizer_file()
    with open(vocab_path, 'r', encoding="utf-8") as f:
        vocabs = json.load(f)
    raw_vocab: Dict[str, int] = vocabs.get('model', {}).get('vocab', {})
    return raw_vocab


def build_sytem_prompt(functions: List[Dict[str, Any]]) -> str:
    """Build system routing instructions string."""
    lines = [
        "STRICT SYSTEM RULE: You are a function-calling router. "
        "You NEVER answer the user directly.",
        "",
        "Task: choose exactly one function name from the list below "
